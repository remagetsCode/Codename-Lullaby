...
function stepHit(step) {
	switch (step) {
		...
		case 128:
			for (i => u in uiStuff.members)
				FlxTween.tween(u, {alpha: 0.9}, 1 + i * 0.3);

		case 368:
			FlxTween.num(0, 0.25, 1, {onUpdate: function(v) {
				desat.desaturationAmount = v.value;
			}});
		...
		case 1640:
			FlxTween.num(5, 6.5, 5, {onUpdate: function(v) {
				aberration.iTime = v.value;
			}});
			FlxTween.num(0.25, 0.5, 1, {onUpdate: function(v) {
				desat.desaturationAmount = v.value;
			}});
	}
}
