...
function postCreate() {
	...
	pkball.animation.onFinish.add(function(a) {
		switch (a) {
			case "throw":
				pkball.animation.play('idle1');
			case "break1":
				pkball.animation.play('idle2');
			case "break2":
				pkball.animation.play('idle3');
			case "final":
				pkball.kill();
			default: // nothing
		}
	...
	new FlxTimer().start(0.01, () -> {
		for (i in [uiStuff.members[0], uiStuff.members[1], iconBF, iconDAD])
			i.visible = false;
	});
}
...
function stepHit(s) {
	...
	switch (s) {
		...
		case 1281:
			FlxTween.tween(nogega, {
				"scale.x": 3,
				"scale.y": 3,
				x: 580,
				y: 80
			}, 2, {ease: FlxEase.quintInOut});
		...
		case 1620:
			pkball.animation.play('break1');
		case 1665:
			pkball.animation.play('break2');
		case 1710:
...
function measureHit() {
	...
	if (apparitiongf && gfTween == null)
		gfTween = FlxTween.circularMotion(fakegf, fakegf.x, fakegf.y - 18, 20, 0, true, 10, true, {type: 2});
}
