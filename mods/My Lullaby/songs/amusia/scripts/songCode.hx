...
function create() {
	...
	bf.colorTransform.color = FlxColor.ORANGE;
	dad.colorTransform.color = FlxColor.PURPLE;
...
function postCreate() {
	...
	talk = new FlxTypeText(dialogBox.x + 30, dialogBox.y + 30, 680, "aa", 28, false);
	talk.wordWrap = true;
	talk.setFormat(Paths.font("pokefont.ttf"), 24, 0xFF000000);
...
function update(elapsed) {
	if (curStep < 800) {
		switch (iconDAD.curCharacter) {
			...
			case "icon-wigglytuff1":
				vignette.alpha = lerp(vignette.alpha, 0.25, 0.05);
			case "icon-wigglytuff2":
				vignette.alpha = lerp(vignette.alpha, 0.5, 0.05);
			case "icon-wigglytuff3":
				vignette.alpha = lerp(vignette.alpha, 0.75, 0.05);
		}
	}

	if (curCameraTarget == 1 && curStep < 790)
		camera.zoom = lerp(camera.zoom, 1.25, 0.05);
	else if (curStep < 540)
...
function onNoteHit(e) {
	...
	new FlxTimer().start(0.05, () -> {
		modchart.setPercent('vibrate', 0, 0);
	});

	new FlxTimer().start(0.3, () -> {
		cpu.deleteNote(n);
	}); // Idk if this solves the lag, I hope yes   ---- 6 months later i just casually figured out how to fix the performance issue just by using strumline.deleteNote(note) lol

	if (curBeat >= 196) {
		iconDAD.setIcon("icon-wigglytuff3");
		return;
	}
...
function stepHit(s) {
	...
	switch (s) {
		case 11:
			FlxTween.tween(dad, {x: 300}, 1, {ease: FlxEase.cubeOut});

		case 21:
			FlxTween.tween(bf, {x: 1048}, 1, {ease: FlxEase.cubeOut});
		...
		case 140:
			missingno.GLITCH_THR = 0.00001;
		case 272:
			FlxTween.num(0, 0.3, 1, null, (v) -> heat1.intensity = v);
			FlxTween.tween(redStatic, {alpha: 0.7}, 1);
			FlxTween.tween(bStatic, {alpha: 0.1}, 1);
			missingno.GLITCH_THR = 0.0001;
		case 274:
			missingno.GLITCH_THR = 0.001;
		case 276:
			missingno.GLITCH_THR = 0.004;
		case 279:
			missingno.GLITCH_THR = 0.008;
		case 282:
		...
		case 288:
			FlxTween.num(0.3, 0, 0.2, null, (v) -> heat1.intensity = v);
		...
		case 540:
			FlxTween.tween(bStatic, {alpha: 0.7}, 0.5);
		case 545:
		...
		case 813:
			...
			FlxTween.num(6.5, 8, 2, null, (v) -> aberration.iTime = v);
		...
		case 1296:
			FlxTween.num(0, 0.4, 0.6, null, (v) -> heat1.intensity = v);
			FlxTween.tween(redStatic, {alpha: 0.7}, 0.8);
		case 1309:
			FlxTween.tween(redStatic, {alpha: 0.1}, 0.5);
			FlxTween.num(0.4, 0, 0.8, null, (v) -> heat1.intensity = v);
			FlxTween.num(8, 6.5, 2, null, (v) -> aberration.iTime = v);
			dad.scrollFactor.set(0.59, 0.59);

		case 1312:
			FlxTween.tween(vignette, {alpha: 0.4}, 1);
		...
		case 2016:
			FlxTween.tween(bStatic, {alpha: 1}, 1);
			FlxTween.tween(bStatic, {alpha: 0}, 5, {startDelay: 1});
		...
		case 2080:
			dialogue("I just wanted to sing...");
		case 2105:
			dialogue("Why... why... why... can't I sing?");
		case 2135:
			dialogue("Just... Sing... Sing...");
		case 2165:
			dialogue("Why can't I sing? Why? WHY?");
		case 2200:
...
function dialogue(target:String) {
	if(curStep >= 2235) FlxG.sound.play(Paths.sound("confirmMenu"));
	talk.resetText(target);
	talk.start(0.04);
}